...
public:
    ...
    glm::vec3 Sample(float t) const {
        if (Points.size() < 4) {
            WK_CORE_WARNING("Spline3D::Sample called with fewer than 4 control points ({0}) - returning origin",
                            Points.size());
            return glm::vec3(0);
        }
        ...
        return 0.5f * ((2.0f * p1) + (-p0 + p2) * localT + (2.0f * p0 - 5.0f * p1 + 4.0f * p2 - p3) * tt +
