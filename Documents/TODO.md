...
- [x] Finish or delete the Terrain/MarchingCubes pathway. **Done, via Mechtrix, not this
      repo:** `MarchingCubes::Generate` is now fully implemented
      (`Terrain/MarchingCubes.cpp`, vertex welding + packed normals), and Mechtrix's
      `Gameplay/Terrain/VoxelWorld.cpp` wires it into a real streaming chunk pipeline
      (generation, meshing, GPU upload, eviction — see `docs/VoxelJobSystemTODO.md` in
      Mechtrix). `SplineCarver.h`/`AsteroidGenerator.h`/`SplineGenerator.h` remain unwired
      scaffolding in Mechtrix specifically for the not-yet-built Asteroid generation model
      (`docs/TODO.md` item 3 there) — that's expected/tracked, not this item's concern.
...
Physics:
- [ ] `SpatialHashGrid`'s cell size is now tuned per-grid rather than one single hardcoded
      constant (`PhysicsSystem` runs a 1.0f dynamic grid and a 16.0f static grid — see the
      Triangle mesh collision entry above), which already resolves the large-static-terrain
      case this item originally flagged. Still genuinely open: the fixed 3x3x3 neighbor
      query itself (`SpatialHashGrid.h`) — a *dynamic* collider larger than one dynamic-grid
      cell (1.0f) can still miss overlaps with things more than one cell away. No dynamic
      collider in the game is currently that large, so this is latent, not live.
- [x] **Triangle mesh collision — done, and further along than originally planned.**
      `Physics/Collision/NarrowPhase/SphereMeshCollision.cpp`/`AABBMeshCollision.cpp`/
      `CapsuleMeshCollision.cpp` all exist (Sphere/AABB/Capsule vs `MeshCollider`, brute-force
      per-triangle as recommended, no BVH) and are wired into `CollisionDispatcher`. Feeds
      from `MeshCollider{Ref<TriangleMesh> Mesh, Offset}` — a flat, loader-agnostic
      `(positions, indices)` contract as intended, used for Mechtrix's voxel terrain today.
      `SpatialHashGrid`'s multi-cell-insertion wrinkle was also solved: `PhysicsSystem` now
      runs two grids (`m_DynamicGrid` at a 1.0f cell size, `m_StaticGrid` at 16.0f, sized for
      typical terrain-chunk extents), and `MeshCollider` entities are indexed via
      `InsertAABB()` (spans every cell their world bounds touch) rather than the
      single-center-point `Insert()` regular colliders use. **Still genuinely open (the
      collision-only slice of this item, not terrain):** the analogous mesh-vs-mesh/glTF
      static-prop case this item also mentioned — nothing today attaches a `MeshCollider` to
      an arbitrary loaded glTF asset outside of voxel terrain, only the terrain code path is
      exercised.
...
| Threading | Basic | `JobSystem` (`Core/JobSystem.{h,cpp}`) exists — priority queue, `hardware_concurrency()-1` workers, main-thread queue for GPU-touching work. Drives Mechtrix's async chunk generation/meshing pipeline (see `docs/VoxelJobSystemTODO.md` there). No exception safety in `WorkerLoop` yet — see Consolidated Findings item 1 in `docs/review02.md` |
