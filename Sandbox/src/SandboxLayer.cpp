...
static const char* MotionAxisLabels[] = {"X", "Y", "Z", "Pitch", "Yaw", "Roll"};
...
SandboxLayer::SandboxLayer() : Layer("Cube") {
    ...
    camTransform.LocalOrientation = glm::angleAxis(glm::radians(0.0f), glm::vec3(1, 0, 0)) *
...
void SandboxLayer::OnUpdate() {
    ...
    auto sphereColliderView = m_Scene.Registry().view<Transform, SphereCollider>();
                    ...
                    for (int input = 0; input < MeshAnimation::AxisCount; input++) {
                        ...
                        for (int output = 0; output < MeshAnimation::AxisCount; output++) {
                            ...
                            if (ImGui::TreeNode(label.c_str())) {
                                ...
                                ImGui::Separator();
                                auto values = Wankel::SecondOrderPreview::GetStepResponse(link.Frequency, link.Damping,
                                                                                          link.Response);
                                ImGui::PlotLines("Step Response", values.data(), (int)values.size(), 0, nullptr, -0.5f,
                                                 2.0f, ImVec2(0, 100));
                        ...
                        ImGui::Combo(comboId.c_str(), &selectedOutputAxis[input], MotionAxisLabels,
                                     MeshAnimation::AxisCount);
